...
extern uint32_t T0up_cpu_bbp[];
extern uint32_t T0dn_cpu_bbp[];
extern uint32_t T1up_cpu_bbp[];
extern uint32_t T1dn_cpu_bbp[];
extern uint32_t T2up_cpu_bbp[];
extern uint32_t T2dn_cpu_bbp[];
extern uint32_t T3up_cpu_bbp[];
extern uint32_t T3dn_cpu_bbp[];
...
__global__
void quark_groestl512_gpu_hash_64(uint32_t threads, uint32_t startNounce, uint32_t *g_hash, uint32_t *g_nonceVector, int iter)
{
	...
	if ((thread < threads) && ((g_nonceVector==NULL) || (g_nonceVector[thread]>=iter)))
	{
		// GROESTL
		uint32_t message[32];
		uint32_t state[32];
...
__host__
void quark_groestl512_sm20_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order, int iter)
{
	int threadsperblock = 512;

	dim3 grid((threads + threadsperblock-1)/threadsperblock);
	dim3 block(threadsperblock);	
	quark_groestl512_gpu_hash_64<<<grid, block>>>(threads, startNounce, d_hash, d_nonceVector, iter);
}
