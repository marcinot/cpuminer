...
void quark_groestl512_gpu_hash_64_quad(const uint32_t threads, const uint32_t startNounce, uint32_t * g_hash, uint32_t * __restrict g_nonceVector, int iter)
{
#if __CUDA_ARCH__ >= 300
...
void quark_groestl512_cpu_hash_64(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order, int iter)
{
	...
	//if (device_sm[dev_id] >= 300 && cuda_arch[dev_id] >= 300)
		quark_groestl512_gpu_hash_64_quad<<<grid, block>>>(threads, startNounce, d_hash, d_nonceVector, iter);
	//else
		//quark_groestl512_sm20_hash_64(thr_id, threads, startNounce, d_nonceVector, d_hash, order, iter);
}
