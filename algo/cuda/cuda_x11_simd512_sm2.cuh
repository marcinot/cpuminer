...
static void x11_simd512_cpu_hash_64_sm2(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_hash, int order)
{
	...
	uint32_t grid((threads + threadsperblock-1)/threadsperblock);
	uint32_t block(threadsperblock);

	size_t shared_size = 0;
			
	x11_simd512_gpu_hash_64_sm2<<<grid, block>>>(threads, startNounce, (uint64_t*)d_hash, d_nonceVector);
