...
void quark_blake512_gpu_hash_32(uint32_t threads, uint32_t startNounce, uint32_t *g_nonceVector, uint64_t *g_hash, int iter)
{
...
#endif
	{		
		off_t hashPosition = (g_nonceVector != NULL) ? g_nonceVector[thread] : thread;
		...
		buf[11] = 0;
		buf[12] = 0;
		buf[13] = 0x0100000000000000ul;
		buf[14] = 0;
		buf[15] = 0x0001000000000000ul;

		// Ending round
		quark_blake512_compress(h, buf, c_sigma_big, c_u512, 256);
...
__global__ __launch_bounds__(256,4)
void quark_blake512_gpu_hash_80(uint32_t threads, uint32_t startNounce, void *outputHash, uint64_t* c_PaddedMessage80)
{
//#if !defined(SP_KERNEL) || __CUDA_ARCH__ < 500
	uint32_t thread = (blockDim.x * blockIdx.x + threadIdx.x);
	if (thread < threads)
	{
		...
		((uint32_t*)buf)[19] = nounce;// cuda_swab32(nounce);
...
void quark_blake512_cpu_hash_32(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_nonceVector, uint32_t *d_outputHash, int order, int iter)
{
...
#endif
	{
		...
		quark_blake512_gpu_hash_32<<<grid, block>>>(threads, startNounce, d_nonceVector, (uint64_t*)d_outputHash, iter);
	}
...
void quark_blake512_cpu_hash_80(int thr_id, uint32_t threads, uint32_t startNounce, uint32_t *d_outputHash, uint64_t* c_PaddedMessage80)
{
...
#endif
	{
		...
		quark_blake512_gpu_hash_80<<<grid, block>>>(threads, startNounce, d_outputHash, c_PaddedMessage80);
	}
}
...
void quark_blake512_cpu_setBlock_80(int thr_id, uint32_t *endiandata, uint64_t* c_PaddedMessage80)
{
...
#endif
	{
		...
		cudaMemcpy(c_PaddedMessage80, message, sizeof(message), cudaMemcpyHostToDevice);
	}
	CUDA_LOG_ERROR();
}
