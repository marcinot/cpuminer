...
#if defined(AES256) && (AES256 == 1) || true
    #define Nk 8
    #define Nr 14
...
#if (defined(pobhCBC) && pobhCBC == 1) || (defined(ECB) && ECB == 1)
// MixColumns function mixes the columns of the state matrix.
...
#endif // #if (defined(pobhCBC) && pobhCBC == 1) || (defined(ECB) && ECB == 1)
...
static void ReverseUint256c(uint8_t toSwap[], uint8_t swapped[])
{
	for (int i = 0; i < 32; i++)
	{
		int iSource = 31 - i;
...
static uint8_t bytedKey[] = { 0x62, 0xfb, 0x17, 0x77, 0xe4, 0x9d, 0xfb, 0x5a, 0x3e, 0xc1, 0x52, 0x27, 0x3f, 0xbd, 0xab, 0x25, 0xe3, 0xbf, 0x61, 
		0x50, 0x39, 0xe8, 0x47, 0x43, 0x77, 0x0e, 0xbd, 0x0c, 0xa0, 0x63, 0xac, 0x9f };
static uint8_t bytedIV[] = {0x1d, 0x2c, 0x79, 0x3d, 0xed, 0xc2, 0x37, 0x89, 0x72, 0x69, 0x85, 0x9b, 0x34, 0xd3, 0x1d, 0x93	};

int Encrypt_AES_CBC_II(uint8_t uHashIn[], uint8_t uPOBH[])
{
	//Must pad the uHashIn to 48 wide with PKCS7 formatted 0x10's:
	uint8_t in1[]  = { 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
		               ...
		               0x0, 0x0, 0x0, 0x0, 0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,
  	                   0x0, 0x0, 0x0, 0x0, 0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,
				   	   0x0, 0x0, 0x0, 0x0, 0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0};
	// Sha hash is reversed first:
	uint8_t uRev[32] = {0x0};
	ReverseUint256c(uHashIn, uRev);
	//	uint8_t in2[32] = {0x0};
	memcpy(in1, uRev, 32);
    struct AES_ctx ctx;
    AES_init_ctx_iv(&ctx, bytedKey, bytedIV);
    AES_CBC_encrypt_buffer(&ctx, in1, 48);
	// First 48 bytes are the result used in POBH
	memcpy(uPOBH, in1, 48);
}
...
void test_suite()
{
	...
	// This tests the AES256 library to ensure its sane.
	#if defined(AES256)
    printf("\nTesting AES256\n\n");
	#elif defined(AES192)
    printf("\nTesting AES192\n\n");
